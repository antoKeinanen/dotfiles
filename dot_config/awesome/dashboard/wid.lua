...
M.blu:buttons(gears.table.join(
  awful.button({}, 1, function()
    ...
    if blue then
      ...
      M.blu:get_children_by_id("blu")[1].markup = "󰂯"
      M.blu.fg = on
    else
      awful.spawn.with_shell("bluetoothctl power off")
      M.blu:get_children_by_id("blu")[1].markup = "󰂲"
      M.blu.fg = off
    end
  end)
))
...
awesome.connect_signal('vol::value', function(mut, val)
  if mut == 0 then
    M.vol:get_children_by_id("vol")[1].markup = "󰕾"
    M.vol.fg = on
    vol = true
  else
...
awesome.connect_signal("net::value", function(_, stat)
  if stat:match("up") then
    M.wifi.fg = on
    M.wifi:get_children_by_id("wifi")[1].markup = "󰖩"
    wifi = true
  else
    M.wifi.fg = off
    M.wifi:get_children_by_id("wifi")[1].markup = "󰖪"
    wifi = false
  end
end)
...
M.pow:buttons(gears.table.join(
  awful.button({}, 1, function()
    awful.spawn("~/.config/powermenu/powermenu.sh shutdown")
  end)
))

M.res:buttons(gears.table.join(
  awful.button({}, 1, function()
    awful.spawn("~/.config/powermenu/powemenu.sh reboot")
  end)
))

M.lck:buttons(gears.table.join(
  awful.button({}, 1, function()
    awful.spawn("~/.config/powermenu/lock.sh")
  end)
))
